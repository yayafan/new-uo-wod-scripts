...
CanWalk(movemode, x1, y1, z1, x2_or_dir, y2 := CANWALK_DIR, realm);
...
CreateItemAtLocation( x, y, z, objtype, amount := 1, realm );
CreateItemCopyAtLocation(x, y, z, item, realm);
...
CreateMenu( title );
CreateMultiAtLocation( x, y, z, objtype, flags := 0, realm );
CreateNpcFromTemplate( template, x, y, z, override_properties := 0, realm);
DestroyItem( item );
DestroyMulti( multi );
Detach();
...
FindPath( x1, y1, z1, x2, y2, z2, realm, flags := FP_IGNORE_MOBILES, searchskirt := 5 );
...
GetHarvestDifficulty( resource, x, y, tiletype, realm );
GetMapInfo( x, y, realm );
...
GetRegionLightLevelAtLocation( x, y, realm );
GetRegionName( object );
GetRegionNameAtLocation( x, y, realm );
GetRegionString( resource, x, y, propertyname, realm );
GetSpellDifficulty( spellid );
GetStandingHeight( x, y, startz, realm );
GetStandingLayers( x, y, flags := MAPDATA_FLAG_ALL, realm );
GetWorldHeight( x, y, realm );
GrantPrivilege( character, privilege );
HarvestResource( resource, x, y, b, n, realm ); // returns b*a where 0 <= a <= n
IsStackable( item1, item2 ); // checks if items can stack
ListAccounts( );
ListEquippedItems( who );
ListGhostsNearLocation( x, y, z, range, realm );
ListHostiles( character, range := 20, flags := 0 );
ListItemsAtLocation( x, y, z, realm );
ListItemsNearLocation( x, y, z, range, realm );
ListItemsNearLocationOfType( x,y,z, range, objtype, realm );
ListItemsNearLocationWithFlag( x,y,z, range, flags, realm );
ListMobilesInLineOfSight( object, range );
ListMobilesNearLocation( x, y, z, range, realm );
ListMobilesNearLocationEx( x,y,z, range, flags, realm );
ListMultisInBox( x1,y1,z1, x2,y2,z2, realm );
ListObjectsInBox( x1,y1,z1, x2,y2,z2, realm );
ListStaticsAtLocation( x, y, z, flags := 0, realm );
ListStaticsInBox( x1,y1,z1, x2,y2,z2, flags := 0, realm );
ListStaticsNearLocation( x, y, z, range, flags := 0, realm );
MoveItemToContainer( item, container, x := -1, y := -1 );
MoveItemToSecureTradeWin( item, character );
MoveObjectToLocation( object, x, y, z, realm, flags := MOVEOBJECT_NORMAL );
...
PlayMovingEffectXYZ( srcx, srcy, srcz, dstx, dsty, dstz, effect, speed, loop := 0, explode := 0, realm );
PlayMovingEffectXYZEx( srcx, srcy, srcz, dstx, dsty, dstz, realm, effect, speed, duration := 0, hue := 0, render := 0, fixeddirection := 0, explode := 0, effect3d := 0, effect3dexplode := 0, effect3dsound := 0 );
...
PlaySoundEffectXYZ( x, y, z, effect, realm );
PlayMusic( chr, music_id := 0 );
PlayStationaryEffect( x, y, z, effect, speed, loop := 0, explode := 0, realm );
PlayStationaryEffectEx( x, y, z, realm, effect, speed, duration := 0, hue := 0, render := 0, effect3d := 0 );
